...
**Solution:**  
I aligned all machines to Eastern Time (America/New_York). On Ubuntu I used `timedatectl`, and on Windows I used `Set-TimeZone` + `w32tm /resync`. After restarting Splunk, timestamps became accurate.  

**Key Takeaway:**  
Always verify timezones and NTP sync early. Time drift breaks correlation and makes dashboards unreliable.  
...
**Solution:**  
- Set the Preferred DNS on the internal adapter to the DC’s IP (192.168.10.9)  
...
**Key Takeaway:**  
Domain join issues in VirtualBox labs are almost always DNS or network profile related, not permissions.  
...
- Created dedicated indexes (`windows`, `client`,`client_sysmon` and `windows_sysmon`)  
...
**Solution:**  
- Switched from `stats` to `timechart`  
- Changed panel visualizations from Table to Column/Bar charts and Single Value panels  
